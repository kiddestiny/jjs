...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        await getUA()
        await get_secretp()
        res = await travel_collectAtuoScore()
        if ( res && res.code===0 && res['data']  && res['data']['result'] ) {
            console.log(`收取：${res['data']['result']['produceScore']}`)
        }
        await $.wait(1000)
    }
...
function travel_collectAtuoScore() {
    let body = {"ss": JSON.stringify({"extraData":{"log":"","sceneid":"HYJhPageh5"},"secretp":secretp,"random":randomNum(8)})}
