...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if ( res && res.inviteId ) {
            inviteIdArr.push(res.inviteId)
            console.log(`助力码：${res.inviteId}`)
        }else console.log("已完成任务")
        await $.wait(1000)
    }

    inviteId="";
    $.newShareCodes = inviteIdArr;
    for (let i = 0; i < cookiesArr.length; i++) {
        cookie = cookiesArr[i];
        $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
        $.index = i + 1;
        await getUA()
        await get_secretp()
        for (let u = 0; u < $.newShareCodes.length && true; ++u) {
            ...
            if ( res && res['code']===0 && res['data'] ) {
                ...
                if( result ){
                    if ( result['alreadyAssistTimes']>=result['maxAssistTimes'] ) {
                        console.log(`助力次数已耗尽，跳出`)
                        break
                    }
                }
...
function travel_collectScore(inviteId) {
        ...
        $.post(taskPostUrl("travel_collectScore", body), async(err, resp, data) => {
            try {
                if (err) {
                    ...
                    if (safeGet(data)) data = JSON.parse(data);
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.code === 0) {
                            ...
                            console.log(`getTaskDetail失败:${JSON.stringify(data)}\n`)
                        }
                    }
                }
            } catch (e) {
